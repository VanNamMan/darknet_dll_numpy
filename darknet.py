...
parse.add_argument("-d","--device",type=str,help="choose device CPU or GPU")
...
def making(image,detections):
    ...
    for detection in detections:
        ...
        cv2.rectangle(img,boundingBox[0],boundingBox[2],color,2)
        cv2.putText(img,label,boundingBox[0],font,1, color,2)
...
def load_net(configPath = "./cfg/yolov3.cfg", weightPath = "./weight/yolov3.weights", metaPath= "./cfg/coco.data"):
    """
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ret,img = cap.read()
        if ret:
            detections = detect_np(netMain, metaMain, img)
            img = making(img,detections)
            cv2.imshow("",img)
